...
Uses label-embedded CSVs from prepare_split.py (split at midnight Feb 16).
...
def load_data(raw_cfg: dict, channels: list[str] | None = None):
    """Load sensor + labels from label-embedded train/test CSVs.

    Reads CSVs generated by prepare_split.py (split at midnight Feb 16).
    Each CSV contains sensor columns + occupancy_label column.
    Zero data leakage — each CSV is physically separated.
    ...
    logger.info(
        "Label-embedded CSVs: train=%d rows (%d labeled), "
