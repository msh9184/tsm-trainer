...
# ============================================================================
# Data loading — label-embedded CSVs from prepare_split.py
# ============================================================================

@dataclass
class SplitData:
    """Container for train/test data loaded from label-embedded CSVs."""
...
def load_split_data(
    ...
    """Load train/test data from label-embedded CSVs.

    Each CSV contains sensor columns + occupancy_label column,
    generated by data/prepare_split.py.  Physically separated files
    ...
    train_sensor, train_labels, ch_names, train_ts = load_labeled_csv(
        data_cfg["train_csv"], label_column=label_col, channels=ch,
    )
    test_sensor, test_labels, _, test_ts = load_labeled_csv(
        data_cfg["test_csv"], label_column=label_col, channels=ch,
    )
...
def generate_summary(all_results: list[dict], output_dir: Path):
    ...
    summary = {
        ...
        "split_method": "label-embedded CSVs (split at midnight Feb 16)",
...
def main():
    ...
    logger.info("Split method: label-embedded CSVs (prepare_split.py, midnight Feb 16)")
